...
class TestInstaller(TestCase):
    ...
    def test_local_paths(self, mock_is_git_repository):
    ...
    def test_can_install_locally_when_files_do_not_exist(self, mock_file_open, mock_path_exists, mock_is_git_repository, mock_run):
        installer = git_xltrail.Installer(mode='local', path='\\path\\to\\repository')
        installer.install()
    ...
    def test_can_install_locally_when_files_exist(self, mock_file_open, mock_path_exists, mock_is_git_repository, mock_run):
        installer = git_xltrail.Installer(mode='local', path='\\path\\to\\repository')
        installer.install()
        mock_run.assert_called_once_with(['git', 'config', 'diff.xltrail.command', 'git-xltrail-diff.exe'], cwd='\\path\\to\\repository', stderr=-1, stdout=-1, universal_newlines=True)
        mock_file_open.assert_has_calls([
